...
// 
// To correct the barometer for  altitude above sea level use the 10 turn potentiometer to adjust the meter to read what your local barormeter setting is. That can be obtained from your local airport or cell phone weather data
// //
...
//Rev 5 - Turn on all 4 leds at startup for ten sec then off . Fixed yellow LED always stuck on 
/* LED Descriptions 
 * Red led on means pressure dropping
 * Green led on means pressure is rising
 * Yellow Led on means pressure is static - not changing
 * Blue led on means a large change was detected . It will be accompanied by eith a red or green led. Blue = pressure change was > 0.06 in of HG
 */
...
double old=29.92 ; 
double newx=29.92;
int Gled = 4 ; //green led on pin D4
int Rled = 5; //Red led on pin D5 
int Yled = 2; //yellow led on D2
int Bled = 3; //blue led on D3
...
pinMode(Yled,OUTPUT);
pinMode(Bled,OUTPUT);
//Turn all the Leds on for 10 sec 
