...
int Gled = 2 ; //green led on pin D2
int Rled = 3; //Red led on pin D3 
int Yled = 4; //yellow led on D4
int Bled = 5; //blue led on D5
...
pinMode(Bled,OUTPUT);
 ...
 float p = analogRead(A0); //Potientiometer input for altitude offset
 float Offset = ((p/1024)*5)-1; //map pot to in of Hg ofset              
 float Q=(bmp.readPressure()); //read the pressure from the barometer in pascals 
 float  PS = Q*0.0002953;  //convert pressue pascals to in"
 float PSNEW = PS;
...
// Below is the LED control if we want to set LEDs for pressure change over the last hour
 ...
 float old=newx; // Shift the pressure 
